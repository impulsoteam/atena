...
const link_auth = `https://disqus.com/api/oauth/2.0/authorize/?client_id=${process.env.DISQUS_PUBLIC_KEY}&scope=email&response_type=code&redirect_uri=${process.env.DISQUS_CALLBACK_URL}`
...
Portanto, tens o que é preciso para estar entre nós, ${user &&
  ...
  if (user && user.disqusUsername) {
    string = `Olá! Leal, ${user.name}, você já pode participar fazendo comentários no nosso blog!`
  }
